...
String.prototype.ezEndsWith = function(suffix) {
    return this.indexOf(suffix, this.length - suffix.length) !== -1;
};
String.prototype.ezStartsWith = function(prefix) {
...
$(document).ready(function() {
	...
	function InitEZAutoCorrect(){
		//create array of arrays from wordlist
		userDefinedList = WordList.split("~");
	...
	// Function used to get caret's position in textarea
	...
	function ezAutoCorrectGmail() {		
			
		var $cArea = $("form iframe").contents().find("body");
		if ($cArea.length > 0){		
			$cArea.each(function(i) {						
				if ($(this).data('ezkeyup') == undefined){
					...
					$(this).keyup(function(e) {													
						...
						if (document.activeElement.tagName == 'IFRAME'){
							...
							var selRange = selObj.getRangeAt(0);
							selCont = selRange.startContainer;
							caretPosition =  findNode(selObj.anchorNode.parentNode.childNodes, selObj.anchorNode) + selObj.anchorOffset;	
							caretPosition = caretPosition - 1;
							
							//caretPosition = selRange.startOffset;
						}
						...
						console.log('********************EZAUTOCORRECT************************');
				  
						var valueOfField = selCont.data;
						
						console.log('caretPosition = ' + caretPosition + '   valueOfField = |' + valueOfField + '|');
											
						var stringUptoCaretPosition = (valueOfField).substr(0, caretPosition);
						if (" " == stringUptoCaretPosition.charAt(caretPosition - 1)) {
							if (AutoPeriod === '1') {
								if (stringUptoCaretPosition.length > 1 && !stringUptoCaretPosition.ezEndsWith('. ')){
									var ezperiod = stringUptoCaretPosition.substr(0, stringUptoCaretPosition.length - 1) + '.';
									var stringFromCaretPositionUptoEnd = (valueOfField).substr(caretPosition);
									// Set new value of field
									selCont.data = ezperiod + stringFromCaretPositionUptoEnd;
									// Set caret's position
									setCaretPosition(selCont, stringUptoCaretPosition.length + 1);						
								}								
							}						
							return;
						}
						
						console.log('stringUptoCaretPosition = |' + stringUptoCaretPosition + '|');
						...
						console.log('lastIndexOfArray = ' + lastIndexOfArray + '   stringToSearch = |' + stringToSearch + '|');
						
						var NeedToCap = false
						try
						{
							if (AutoCap === '1'){
								if (lastIndexOfArray > 0 && (stringToArray[lastIndexOfArray - 1].ezEndsWith('.') || stringToArray[lastIndexOfArray - 1].ezEndsWith('?') || stringToArray[lastIndexOfArray - 1].ezEndsWith('!') )){
									NeedToCap = true;							 
								}
								//else if (lastIndexOfArray == 0) {
								//	NeedToCap = true;
								//}
							}
						}catch (errrr){}
						console.log('NeedToCap = ' + NeedToCap);
							
						//handle punctuation...
						
						stringToSearch = stringToSearch.ezTrim();
						var suffixPunct = '';
						
						if (stringToSearch.length > 0 && (stringToSearch.ezEndsWith('.') || stringToSearch.ezEndsWith('?') || stringToSearch.ezEndsWith('!') )){
							suffixPunct = stringToSearch.substr(stringToSearch.length - 1, stringToSearch.length);
							
							stringToSearch = stringToSearch.substr(0, stringToSearch.length - 1);
						}
						
						console.log('suffixPunct = ' + suffixPunct + '     search for = |' + stringToSearch + '|');
						
						
						var stringToReplace = stringToSearch;
						var IsFound = false;
						var l = userDefinedList.length;
						for (i=0; i<l; i++) {
							var re = userDefinedList[i][0];
							if (re.toUpperCase() === stringToSearch.toUpperCase() &&  stringToSearch != userDefinedList[i][1] ){
								stringToReplace = userDefinedList[i][1];
								
								if (stringToSearch.match(/\b[A-Z]{2,}\b/)){
									stringToReplace = stringToReplace.toUpperCase();
								}
								else if (stringToSearch.isUpper(0)){
									stringToReplace = stringToReplace.toTitleCase();
								}
								
								stringToReplace += suffixPunct;
								IsFound = true;
								break;
							}
						}
						...
						console.log('stringUptoCaretPosition = |' + stringUptoCaretPosition + '|    stringFromCaretPositionUptoEnd = |' + stringFromCaretPositionUptoEnd + '|');

						if (stringFromCaretPositionUptoEnd.length < 1)
							stringFromCaretPositionUptoEnd = ' ';
							
						var dataTotalNew = stringUptoCaretPosition + stringFromCaretPositionUptoEnd;
						
						console.log('dataTotalNew = |' + dataTotalNew + '|    last char = ' + dataTotalNew.charCodeAt(dataTotalNew.length - 1));
						
						/*
						if (dataTotalNew.charCodeAt(dataTotalNew.length - 1) == 160 && stringFromCaretPositionUptoEnd.length > 1)
						{
							dataTotalNew = (dataTotalNew).substr(0, dataTotalNew.length - 1);
							console.log('dataTotalNewCorrected = |' + dataTotalNew + '|    last char = ' + dataTotalNew.charCodeAt(dataTotalNew.length - 1));
						}
						else if (dataTotalNew.charCodeAt(dataTotalNew.length - 1) == 160)
						{
							dataTotalNew = (dataTotalNew).substr(0, dataTotalNew.length - 1) + ' ';
							console.log('dataTotalNewCorrected = |' + dataTotalNew + '|    last char = ' + dataTotalNew.charCodeAt(dataTotalNew.length - 1));
						}
						*/
						...
						// Set new value of field
						//selCont.data = stringUptoCaretPosition + stringFromCaretPositionUptoEnd;
						selCont.data = dataTotalNew;
												
					    console.log('selCont.data = |' + selCont.data + '| length = ' + selCont.data.length);
						var carPosNew = stringUptoCaretPosition.length + 1;
						console.log('new caret pos = ' + carPosNew);
						
						// Set caret's position
						//setCaretPosition(selCont, stringUptoCaretPosition.length + 1);	
						//setCaretPosition(selCont, carPosNew);	
						selObj.collapse(selCont, carPosNew);
						
						console.log('selCont.data = |' + selCont.data + '| length = ' + selCont.data.length);
						console.log('********************END EZAUTOCORRECT************************');						
					
					});

					$(this).data('ezkeyup', 'IsAttached');
				}				
			});
		...
		var $ctextArea = $('textarea[name="body"]');
		if ($ctextArea.length > 0){	
			$ctextArea.each(function(i) {
				if ($(this).data('ezkeyup') == undefined){
					console.log('add keyup event to textarea');
					$(this).keyup(function(e) {	
						...
						if (" " == stringUptoCaretPosition.charAt(caretPosition - 1)) {
							if (AutoPeriod === '1') {
								if (stringUptoCaretPosition.length > 1 && !stringUptoCaretPosition.ezEndsWith('. ')){
									...
									var stringFromCaretPositionUptoEnd = (valueOfField).substr(caretPosition);
									// Set new value of field
									this.value = (ezperiod + stringFromCaretPositionUptoEnd);
									// Set caret's position
						...
						// Set new value of field
						this.value = dataTotalNew;

												
					    console.log('selCont.data = |' + this.value + '| length = ' + this.value.length);
						var carPosNew = stringUptoCaretPosition.length + 1;
						console.log('new caret pos = ' + carPosNew);
						
						// Set caret's position
						setCaretPosition(this, carPosNew);
